    ...
    <Layout>
      ...
      <Content :style="{height:'100vh'}">
          ...
          <Menu mode="horizontal" theme="light" @on-select="login_out">
            ...
            <div class="navBreadcrumb">
              <Breadcrumb>
                ...
                <BreadcrumbItem v-for="(item, key)  in navBreadcrumb" :key="key" to="/">
        ...
        <TabPane label="新用户注册" name="name2">
          <div class="form-con">
          <register-form @on-success-valid="handleRegister"></register-form>
        </div>
        </TabPane>
    </Tabs>
            </div>
  ...
  methods: {
    ...
    onSelect(name) {
      if( !this.$store.state.hasGetUserInfo ) {
       ...
       !this.navCase.some( ele => ele && ele.name === name ) && this.navCase.push( this.get_matched_route(name ))
    ...
    showItemCase() {
      ...
      setTimeout(() => {
        let active_tag_index = this.navCase.findIndex( value  =>  value === this.navThis )  > -1  && this.moveToView(this.refsTag[active_tag_index].$el)
      }, 200);
    },
    ...
    handleClick(item) {
      ...
      this.$nextTick(function() { this.$refs.mymenu.updateOpened() })
